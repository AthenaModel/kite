...
proc ::kiteutils::template {name arglist initbody {template ""}} {
...
proc ::kiteutils::tforeach {vars items initbody {template ""}} {
    # FIRST, have we an initbody?
    if {"" == $template} {
        set template $initbody
        set initbody ""
...
proc ::kiteutils::tif {condition thenbody {"else" "else"} {elsebody ""}} {
...
snit::macro template {proctype name arglist initbody {template ""}} {
    ...
    # NEXT, define the body of the new proc so that the initbody, 
    # if any, is executed and then the substitution is 
    set body "$initbody\n    ::kiteutils::tsubst [list $template]\n"
